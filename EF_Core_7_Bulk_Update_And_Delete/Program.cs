...
//await context.Persons.Where(p => p.PersonId > 3).ExecuteUpdateAsync(p => p.SetProperty(p => p.Name, v => $"{v.Name} yeni")); //String Interpolation ından dolayı hata verir
#endregion
...
class ApplicationDbContext : DbContext
{
    ...
    {
        optionsBuilder
            .UseSqlServer("server=localhost\\SQLEXPRESS;database=ApplicationDb;integrated security=true;TrustServerCertificate=True");
    }
}